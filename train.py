...
def fit_one_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    torch.save(model.state_dict(), 'logs/Epoch%d-Total_Loss%.4f-lr%.7f-Val_Loss%.4f.pth'%((epoch+1),total_loss/(epoch_size+1),double(get_lr(optimizer)),val_loss/(epoch_size_val+1)))
